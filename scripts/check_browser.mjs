...
const server = spawn('npm', ['run', 'preview', '--', '--port', '4174', '--strictPort'], { stdio: 'ignore', cwd: new URL('..', import.meta.url) })
...
try {
  ...
  browser = await chromium.launch({ channel: process.env.BROWSER_CHANNEL || 'chrome', args: ['--enable-unsafe-swiftshader'] })
  const page = await browser.newPage({ viewport: { width: 1440, height: 1000 } })
  const errors = []
  page.on('pageerror', error => errors.push(error.message))
  await page.goto(url)
  await expect(page.locator('.total strong')).toHaveText('724,638')
  await expect(page.getByRole('button', { name: 'Reset view' })).toBeEnabled({ timeout: 30000 })
  ...
  await expect(mobile.getByRole('button', { name: 'Reset view' })).toBeEnabled({ timeout: 30000 })
  ...
  await mobile.touchscreen.tap(mb.x + mb.width * .6, mb.y + mb.height * .4)
  ...
  await expect(failed.getByRole('alert')).toContainText('Election data could not be loaded')
  await expect(failed.getByRole('button', { name: 'Reload', exact: true })).toBeVisible()
  const noStyle = await browser.newPage()
  await noStyle.route('https://tiles.openfreemap.org/styles/positron', route => route.abort())
  await noStyle.goto(url)
  await expect(noStyle.getByRole('alert')).toContainText('Street map unavailable')
  await expect(noStyle.getByRole('button', { name: 'Reset view' })).toBeEnabled()
  const noTiles = await browser.newPage()
  await noTiles.route('**/*.pbf', route => route.abort())
  await noTiles.goto(url)
  await expect(noTiles.getByRole('alert')).toContainText('Some map resources could not load', { timeout: 30000 })
  await expect(noTiles.locator('.total strong')).toHaveText('724,638')
  console.log('PASS: production desktop/mobile, election switches, map clicks/taps, and data/style/tile failures')
} finally {
  await browser?.close()
  server.kill('SIGTERM')
}
